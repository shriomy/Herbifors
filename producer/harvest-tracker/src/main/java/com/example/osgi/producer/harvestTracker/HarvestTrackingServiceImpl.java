...
public class HarvestTrackingServiceImpl implements HarvestTrackingService, BundleActivator {
    ...
    public void start(BundleContext context) {
        ...
        System.out.println("✅ HarvestTrackingService started.");
    }

    @Override
    public void stop(BundleContext context) {
        registration.unregister();
        System.out.println("🛑 HarvestTrackingService stopped.");
    }
    ...
    public String getHarvestData() {
        ...
        for (Crop crop : crops.values()) {
            data.append("🌾 ").append(crop.getQuantity()).append("kg ").append(crop.getName())
                    .append(" @ $" ).append(crop.getPrice()).append("/kg\n");
        }
        return data.toString();
    }

    @Override
    public void addCrop(String name, int quantity, double price) {
        crops.put(name, new Crop(name, quantity, price));
    }

    @Override
    public void updateCropQuantity(String name, int quantity) {
        if (crops.containsKey(name)) {
            crops.get(name).setQuantity(quantity);
        }
    }

    @Override
    public void deductCrop(String name, int quantity) {
        if (crops.containsKey(name)) {
    ...
    @Override
    public Map<String, Crop> getCropDetails() {
        return crops;
    }
